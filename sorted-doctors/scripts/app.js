let MC = {}; // You can totally put React components into a parent object.
...
MC.tables.Sorted = class extends React.Component {
  ...
  _getSortedArray(arrItems, strOrderBy = "number") {
    let arrSorted =  _.sortBy(arrItems, function(item){ return item[strOrderBy] });
  ...
  _getRows() {
    let that = this;
    return this._getSortedArray(this.props.data.rows, this.state.sortType).map((row, i) => {
      return(
        <tr key={ i }>
          { this.props.data.columns.map( (column,j)=>{return(<td key={j}>{row[column.sortType]}</td>);} ) }
        </tr>
      );
    });
  }
...
MC.tables.SortHeader = class extends React.Component {
  ...
  render() {
    ...
    return(
      <th key={this.props.key} className={activeClasses} scope="col" onClick={() => this._handleClick()}>{this.props.text} <i className={"fas " + this.props.iconClass + sortClass}></i></th>
...
ReactDOM.render(
  <MC.tables.Sorted data={tableData} />, document.getElementById('magic-table')
);